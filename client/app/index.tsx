...
const HomeScreen = ({ navigation }) => {
  ...
  const fetchCategories = async () => {
    try {
      const response = await axios.get(`http://192.168.1.2:5000/categories`);
  ...
  const fetchProducts = async (categoryId) => {
    try {
      const response = await axios.get(`http://192.168.1.2:5000/products`, {
  ...
  useEffect(() => {
    const fetchWishlist = async () => {
      if (userId && token) {
        try {
          const response = await axios.get(`http://192.168.1.2:5000/wishlists/${userId}`, {
  ...
  const toggleWishlist = async (productId) => {
    ...
    try {
      if (isInWishlist) {
        await axios.delete(
          `http://192.168.1.2:5000/wishlists/${userId}/${productId}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
        ...
        await axios.post(
          `http://192.168.1.2:5000/wishlists/`,
  ...
  const handleAddToCart = async (productId) => {
    ...
    try {
      await axios.post(
        `http://192.168.1.2:5000/carts/add`,
